...
- **Tunneling**: Exposes the local FastAPI server to the internet using a persistent Cloudflare Tunnel (`bot.csydev.online`).
...
The bot features a highly secure SQLite authentication system. A root `ADMIN_ID` (1000808626) is hardcoded to ensure the owner always has access, bypassing any file corruption or `.env` issues.
...
- **Command Menu Scoping**: Regular users only see a clean, 3-item command menu (`/start`, `/help`, `/audio`). Admin-level commands are programmatically restricted to the Admin's private chat using `BotCommandScopeChat`, ensuring zero clutter for end-users.
